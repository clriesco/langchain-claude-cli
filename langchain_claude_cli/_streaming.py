...
class _StreamingMixin:
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        # Session rounds: at most one restart, when the resumed session
        # turns out to be purged from the CLI (stale mapping) — mirror of
        # the invoke path in _runner.py.
        while True:
            ...
            try:
                            ...
                            if dtype == "text_delta":
                                ...
                                if text:
                                    ...
                                    if (
                                        ...
                                        text = text[
                                            : self.max_tokens * 4 - emitted_chars
                                        ]
                                        hit = True
                                    chunk = ChatGenerationChunk(
                ...
                if (
                    ...
                    and not resolution.explicit
                    and _is_stale_session_error(e, stale_stderr)
                ):
                    # The CLI purged this session's transcript: re-run as a
                    # new session (nothing was emitted yet, so no chunk is
                    # duplicated). An explicitly pinned session propagates.
                ...
                if session_id and not stopped:
                    ...
                    self._session_cache.register(
                        [
                            *messages,
                            AIMessage(
                                content=cast(Any, content), tool_calls=tool_calls
                            ),
                        ],
                        session_id,
